...
- total bytes：50,305,498
- file count：2,515
- HTML：2,488
- JSON：20
- JS bytes：15,144
- CSS bytes：21,239
- search index：598,403 bytes
- smoke ordinary image count：20,490；未发现普通 `<img>` 指向 original/upscaled full Object。
...
| `npm test` | exit 0；60 passed，0 failed，0 skipped，0 todo |
| `npm run site:check` | exit 0；39 files，0 errors，0 warnings，0 hints |
| `npm run site:build` | exit 0；2,488 page(s) built；无 Astro/build warning |
