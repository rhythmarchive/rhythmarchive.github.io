...
export function GET(): Response {
  ...
  const paths = ["/", "/search/", "/feedback/", ...data.games.map((game) => `/${game.slug}/`), ...data.games.flatMap((game) => game.categories.map((category) => `/${game.slug}/${category.slug}/`)), ...data.resources.map((resource) => resource.route)];
