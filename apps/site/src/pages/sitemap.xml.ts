...
export function GET(): Response {
  ...
  const paths = ["/", "/games/", "/search/", "/updates/", "/feedback/", ...data.games.flatMap((game) => game.categories.map((category) => `/${game.slug}/${category.slug}/`)), ...data.resources.map((resource) => resource.route), ...data.updates.map((update) => `/updates/${update.id}/`)];
