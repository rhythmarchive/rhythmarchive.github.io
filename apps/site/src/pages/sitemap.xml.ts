...
export function GET(): Response {
  ...
  const paths = ["/", "/games/", "/search/", "/feedback/", ...data.games.flatMap((game) => game.categories.map((category) => `/${game.slug}/${category.slug}/`)), ...data.resources.map((resource) => resource.route)];
