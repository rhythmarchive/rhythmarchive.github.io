(() => {
    ...
    page().innerHTML = `
      ...
      <form class="panel" data-form="new-workspace">
        <div class="form-grid">
          ...
          <div class="form-field full"><label><input type="checkbox" name="compareSameSource" /> Run a no-op comparison with one APK (0 binary changes, no upload plan)</label></div>
  ...
  function renderReview() {
    ...
    page().innerHTML = `
      <div class="panel diff-summary"><span>Added ${diff.added || 0}</span><span>Changed ${diff.contentChanged || 0}</span><span>Metadata ${diff.metadataOnly || 0}</span><span>Needs confirmation ${diff.unmatched || 0}</span><span>Removed from source ${diff.removed || 0}</span></div>
  ...
  // Phase 6 review renderer: keep the existing Admin shell but expose the
  // workspace-local correction actions and an original/upscaled comparison.
  function renderCandidateCard(candidate) {
    const active = candidate.disposition === "active" && candidate.status !== "REJECTED";
    const canFinalize = active && candidate.confirmed && candidate.status !== "READY" && (!candidate.needsUpscale || candidate.upscale.converted);
    const status = candidate.disposition !== "active" ? candidate.disposition : (statusNames[candidate.status] || candidate.status);
    const issueHtml = candidate.issues.length ? `<div class="candidate-issues">${candidate.issues.map((issue) => `<div class="issue">${escapeHtml(issue)}</div>`).join("")}</div>` : "";
    const original = `<img src="${escapeHtml(candidate.originalPreviewUrl)}" alt="original" loading="lazy" />`;
    const upscale = candidate.upscaledPreviewUrl ? `<img src="${escapeHtml(candidate.upscaledPreviewUrl)}" alt="upscaled" loading="lazy" />` : "<span class=\"preview-placeholder\">no upscale</span>";
    const categoryOptions = Object.keys(resourceNames).map((key) => `<option value="${escapeHtml(key)}" ${candidate.resourceType === key ? "selected" : ""}>${escapeHtml(resourceNames[key] || key)}</option>`).join("");
    const override = `<details class="override" ${candidate.needsInfo ? "open" : ""}><summary>Edit metadata</summary><form class="override-form" data-form="override" data-candidate-id="${escapeHtml(candidate.id)}"><label class="field-label">Title</label><input name="title" value="${escapeHtml(candidate.title || candidate.suggestedTitle || "")}" /><label class="field-label">Artist</label><input name="artist" value="${escapeHtml(candidate.artist || "")}" /><label class="field-label">Category</label><select name="category">${categoryOptions}</select><label class="field-label">Download filename</label><input name="filename" value="${escapeHtml(candidate.filename || "")}" /><button class="button button-small" type="submit">Save</button></form></details>`;
    const finalizeLabel = candidate.target ? "Finalize update" : "Add as new";
    const reviewButtons = active ? `${!candidate.confirmed ? `<button class="button button-primary button-small" data-action="confirm" data-candidate-id="${escapeHtml(candidate.id)}">Confirm</button>` : ""}${canFinalize ? `<button class="button button-small" data-action="finalize-new" data-has-target="${candidate.target ? "true" : "false"}" data-candidate-id="${escapeHtml(candidate.id)}">${finalizeLabel}</button>` : ""}<button class="button button-small" data-action="replace-image" data-candidate-id="${escapeHtml(candidate.id)}">Replace image</button><button class="button button-small" data-action="remove-candidate" data-candidate-id="${escapeHtml(candidate.id)}">Remove from update</button><button class="button button-small" data-action="ignore-candidate" data-candidate-id="${escapeHtml(candidate.id)}">Ignore resource</button>` : `<button class="button button-small" data-action="restore-candidate" data-candidate-id="${escapeHtml(candidate.id)}">Restore extractor result</button>`;
    return `<article class="candidate-card ${candidate.status === "BLOCKED" ? "is-blocked" : ""}"><div class="asset-preview comparison-preview"><div>${original}<small>original</small></div><div>${upscale}<small>upscaled</small></div></div><div class="candidate-body"><div class="candidate-topline"><h3 class="candidate-title">${escapeHtml(candidate.title || candidate.filename)}</h3>${statusChip(status, candidate.status === "BLOCKED" ? "danger" : candidate.confirmed ? "ok" : "")}</div><div class="candidate-artist">${escapeHtml(candidate.artist || "Artist not identified")}</div><div class="candidate-tags"><span class="tag accent">${escapeHtml(resourceNames[candidate.resourceType] || candidate.resourceType)}</span><span class="tag">${escapeHtml(candidate.filename)}</span>${candidate.modified ? '<span class="tag">modified</span>' : ""}${candidate.replacedImage ? '<span class="tag">image replaced</span>' : ""}</div>${issueHtml}<div class="candidate-actions">${reviewButtons}<button class="button button-small" data-action="open-folder" data-folder="workspace">Open workspace</button></div>${override}</div></article>`;
  }
  ...
  function renderUpscale() {
    const view = state.currentView;
    if (!view) { page().innerHTML = '<div class="loading">Loading workspace...</div>'; return; }
    const items = view.candidates.filter((candidate) => candidate.needsUpscale || candidate.upscale.matches.length > 0 || candidate.upscale.converted);
    const total = view.candidates.filter((candidate) => candidate.needsUpscale || candidate.upscale.converted).length;
    const complete = view.candidates.filter((candidate) => candidate.upscale.converted).length;
    page().innerHTML = `<div class="page-head"><div><h1>AI upscale</h1><p>Local Real-ESRGAN, one candidate at a time.</p></div><div class="head-actions"><button class="button" data-action="prepare-upscale">Prepare inputs</button><button class="button button-primary" data-action="run-upscale">Start all pending</button><button class="button" data-action="rescan-upscale">Rescan outputs</button></div></div><div class="panel"><div class="section-head" style="margin-top:0"><h2>Progress</h2><span class="muted">${complete} / ${total}</span></div>${items.length ? `<div class="upscale-list" style="margin-top:14px">${items.map(renderUpscaleRowPhase6).join("")}</div>` : '<div class="empty-state" style="margin-top:14px"><strong>No pending jacket upscale</strong><span>Approved Arcaea jackets appear here.</span></div>'}</div>`;
  }

  function renderUpscaleRowPhase6(candidate) {
    const failed = candidate.upscale.state === "needs-upscale" && candidate.issues?.length;
    const actions = candidate.upscale.converted ? `<button class="button button-small" data-action="skip-upscale" data-candidate-id="${escapeHtml(candidate.id)}">Publish original</button>` : failed ? `<button class="button button-primary button-small" data-action="retry-upscale" data-candidate-id="${escapeHtml(candidate.id)}">Retry</button><button class="button button-small" data-action="skip-upscale" data-candidate-id="${escapeHtml(candidate.id)}">Skip upscale</button>` : "";
    return `<div class="upscale-row"><div><div class="upscale-title">${escapeHtml(candidate.title || candidate.filename)}</div><div class="upscale-meta"><span>${escapeHtml(candidate.filename)}</span><span>${escapeHtml(candidate.upscale.state)}</span></div></div><div class="upscale-actions">${actions}</div></div>`;
  }
  ...
  function renderSettings() {
    const config = state.bootstrap.config;
    page().innerHTML = `<div class="page-head"><div><h1>Settings</h1><p>Local paths only; no credentials are stored here.</p></div></div><form class="panel" data-form="settings"><div class="form-grid"><div class="form-field full"><label>Arcaea APK directory</label><input name="arcaeaApkDir" value="${escapeHtml(config.arcaeaApkDir)}" /></div><div class="form-field full"><label>Phigros APK directory</label><input name="phigrosApkDir" value="${escapeHtml(config.phigrosApkDir)}" /></div><div class="form-field full"><label>Runtime directory</label><input name="workspaceRuntimePath" value="${escapeHtml(config.workspaceRuntimePath)}" /></div><div class="form-field full"><label>Extractor root</label><input name="legacyExtractorRoot" value="${escapeHtml(config.legacyExtractorRoot)}" /></div><div class="form-field full"><label>Catalog JSON</label><input name="catalogPath" value="${escapeHtml(config.catalogPath)}" /></div><div class="form-field full"><label>Real-ESRGAN executable</label><input name="realEsrganExecutable" value="${escapeHtml(config.realEsrganExecutable || "")}" placeholder="Leave blank for discovered ArSrNaUIESRGAN install" /></div><div class="form-field full"><label>Real-ESRGAN model directory</label><input name="realEsrganModelDir" value="${escapeHtml(config.realEsrganModelDir || "")}" /></div><div class="form-field full"><label>Model name</label><input name="realEsrganModelName" value="${escapeHtml(config.realEsrganModelName || "realesrgan-x4plus-anime")}" /></div></div><div class="form-actions"><button class="button button-primary" type="submit">Save settings</button></div></form>`;
  }
  ...
  function renderPublish() {
    ...
    page().innerHTML = `<div class="page-head"><div><h1>发布</h1><p>${escapeHtml(view.gameName)} ${escapeHtml(view.targetVersion)}</p></div><div class="head-actions">${publishButton}<button class="button" data-action="generate-publish">生成预览</button></div></div>
      <div class="panel"><div class="inline-note ${configured ? "ok" : "warn"}">${configured ? "ROS 已配置。" : "ROS 凭据未配置。"}</div>${publish ? `<div class="publish-summary"><div class="publish-number"><strong>${publish.summary.addedResources}</strong><span>新增资源</span></div><div class="publish-number"><strong>${publish.summary.updatedResources}</strong><span>更新资源</span></div><div class="publish-number"><strong>${publish.summary.metadataOnly}</strong><span>metadata 更新</span></div><div class="publish-number"><strong>${publish.summary.removedFromCurrentSource}</strong><span>源中缺失（保留）</span></div><div class="publish-number"><strong>${publish.summary.upscaledRenditions}</strong><span>新增超分版</span></div><div class="publish-number"><strong>${publish.summary.originalRenditions}</strong><span>仅原图</span></div><div class="publish-number"><strong>${publish.summary.uploadObjects}</strong><span>待上传 Object</span></div><div class="publish-number"><strong>${formatBytes(publish.summary.uploadBytes)}</strong><span>预计上传</span></div></div><div class="inline-note">不会删除任何历史 Resource 或 ROS Object。</div><details open><summary>变化</summary><div class="change-list">${publish.manifest.changes.length ? publish.manifest.changes.map((change) => `<div class="change-row">${escapeHtml(change.detail)}</div>`).join("") : '<div class="input-note">没有 Catalog 变化。</div>'}</div></details>` : '<div class="empty-state" style="margin-top:16px"><strong>还没有预览</strong><span>先生成预览，确认对象和 Catalog 变化。</span></div>'}</div>${execution ? `<div class="panel"><div class="section-head" style="margin-top:0"><h2>发布进度</h2><span class="tag ok">完成</span></div><div class="change-list">${execution.progress.map((item) => `<div class="change-row">${escapeHtml(item.message)}</div>`).join("")}</div></div>` : ""}`;
  }
  ...
  async function handleAction(button) {
    ...
    if (action === "run-upscale") { const result = await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/upscale/run`, { method: "POST", body: "{}" }); updateView(result); notify("Real-ESRGAN finished", `${result.result.completedCandidateIds.length} completed, ${result.result.failed.length} failed`, result.result.failed.length ? "warn" : "ok"); return; }
    if (action === "retry-upscale") { const result = await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/upscale/retry`, { method: "POST", body: JSON.stringify({ candidateId: button.dataset.candidateId }) }); updateView(result); notify("Retry finished", result.result.failed.length ? "The upscale still failed." : "Upscale completed.", result.result.failed.length ? "warn" : "ok"); return; }
    if (action === "skip-upscale") { updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/upscale/skip-original`, { method: "POST", body: JSON.stringify({ candidateId: button.dataset.candidateId }) })); notify("Original selected", "This candidate can be published without the upscale result.", "ok"); return; }
    if (action === "replace-image") { const sourcePath = window.prompt("Enter the local image path to copy into this update:"); if (!sourcePath) return; updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/candidates/${encodeURIComponent(button.dataset.candidateId)}/replace`, { method: "POST", body: JSON.stringify({ sourcePath }) })); notify("Image replaced", "The previous upscale result was invalidated.", "ok"); return; }
    if (action === "remove-candidate") { if (!window.confirm("Remove this candidate from the current update? Published Catalog and ROS objects are not deleted.")) return; updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/candidates/${encodeURIComponent(button.dataset.candidateId)}/remove`, { method: "POST", body: "{}" })); notify("Removed from update", "No published resource was deleted.", "ok"); return; }
    if (action === "ignore-candidate") { if (!window.confirm("Ignore this extracted resource for this update? Published Catalog and ROS objects are not deleted.")) return; updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/candidates/${encodeURIComponent(button.dataset.candidateId)}/ignore`, { method: "POST", body: "{}" })); notify("Resource ignored", "The decision stays in this update workspace and can be restored.", "ok"); return; }
    if (action === "restore-candidate") { updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/candidates/${encodeURIComponent(button.dataset.candidateId)}/restore`, { method: "POST", body: "{}" })); notify("Extractor result restored", "Review starts again from the original candidate.", "ok"); return; }
    ...
    if (action === "convert-upscale") {
      try {
        updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/upscale/convert`, { method: "POST", body: JSON.stringify({ candidateId: button.dataset.candidateId }) }));
    ...
    if (action === "finalize-new") { const existingTarget = button.dataset.hasTarget === "true"; if (!window.confirm(existingTarget ? "将这个候选作为现有资源的更新加入发布预览？" : "将这个候选标记为新资源并加入发布预览？")) return; updateView(await api(`/api/workspaces/${encodeURIComponent(state.currentWorkspaceId)}/candidates/${encodeURIComponent(button.dataset.candidateId)}/finalize`, { method: "POST", body: JSON.stringify({ createNewTarget: !existingTarget }) })); notify("已加入发布预览", "尚未上传 ROS。", "ok"); return; }
  ...
  async function handleSubmit(form) {
    if (form.dataset.form === "new-workspace") {
      ...
      payload.compareSameSource = data.get("compareSameSource") === "on";
